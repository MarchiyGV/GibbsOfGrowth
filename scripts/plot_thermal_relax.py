...
def main(args):
    ...
    ax2.hlines(0, xmin=t.min(), xmax=t.max(), linestyle='--', color='grey')
    ax1.set_xlabel('$t, ps$')
    ax2.set_xlabel('$t, ps$')
    ...
    ax3.plot(t1, pe1)
    ax4 = ax3.twinx()
    ax4.plot(t1, P1, color='red')
    ax4.hlines(0, xmin=t1.min(), xmax=t1.max(), linestyle='--', color='grey')
    ax3.set_xlabel('$t, ps$')
    ax4.set_xlabel('$t, ps$')
    ...
    df.to_csv(f'../workspace/{name}/thermo_output/{inp}.txt')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dump-step", dest='dump_step', required=False, default=5)
